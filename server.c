...
char usrupdate[] = "Amount has been debited.\n****************\n\nDo you want to continue?";
char usrupdateu[] = "Not Sufficient Amount to Perfrom Required Action\n***************\n\nType exit to quit.";
	...
	if(fcntl(fd, F_SETLK, &fl) == -1){
		sendMsgtoClient(client_fd, "\n\nAnother Transaction is in Process. Action Failed. Please try after sometime. Write back or exit to quit\n");
		close(fd);
		return 0;
	...
	while(1){
		...
		if(amount<=0)
		sendMsgtoClient(client_fd,"Please Enter valid amount");
		else
		   break;
		}
		if(balance >= amount){
	...
	close(fd);
	printf("User DataBase updated\n");
}

// Credit account
int Credit(char *username, char *password, int client_fd){

    char usrupdate[] = "Amount has been credited.\n***************\n\nDo you want to continue?";
...
char usrupdate[] = "Password is updated successfully.\n******************\n\nDo you want to continue?";
...
void adminRequests(char *username, int client_fd)
{
	...
	while(1){
		...
		if(strcmp(buff,"exit")==0)
			...
			case 1:
				AddUser(client_fd);
				strcpy(dat, "New User Added Successfully!\n");
			...
			case 2:
				jointAcc(client_fd);
				strcpy(dat,"Joint Account Added Successfully!\n");
				sendMsgtoClient(client_fd,strcat(dat,options));
				break;
			...
			case 4:
				...
				strcpy(dat,"Account deleted Successfully!\n");
				sendMsgtoClient(client_fd,strcat(dat,options));
				break;
